...
Ny = 30;
%Initial Conditions
bEq = (a + sqrt(a^2 - 4*m^2))/(2*m);
wEq = m/bEq;

rng(10)

w0 = wEq + 0.01*randn(Nx,Ny);
b0 = bEq + 0.01*randn(Nx,Ny);

%Flatten a blank vector to store solutions
u0 = [w0(:);b0(:)];

tspan = linspace(0,200,401);
...
[t,u] = ode15s(@klausmeir,tspan,u0,options);
...
for i = 2:Nx-1
        ...
        %Compute RHS of PDES
        dw(i,j) = a - w(i,j) - w(i,j)*b(i,j)^2 + alpha*lapw(i,j) - alpha*beta*lapb(i,j);
        db(i,j) = w(i,j)*b(i,j)^2 - m*b(i,j) + Db*lapb(i,j);
        end
end
